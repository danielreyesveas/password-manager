var ByteUtils = require("./byte-utils"),
...
function sha256(data) {
	if (!data.byteLength) {
		return Promise.resolve(
			ByteUtils.arrayToBuffer(ByteUtils.hexToBytes(EmptySha256))
		);
	}
	if (subtle) {
		...
		return Promise.reject(
			new KdbxError(
				Consts.ErrorCodes.NotImplemented,
...
function sha512(data) {
	if (!data.byteLength) {
		return Promise.resolve(
			ByteUtils.arrayToBuffer(ByteUtils.hexToBytes(EmptySha512))
		);
	}
	if (subtle) {
...
AesCbcSubtle.prototype.importKey = function (key) {
	var that = this;
	return subtle
		.importKey("raw", key, { name: "AES-CBC" }, false, [
			"encrypt",
			"decrypt",
		])
		.then(function (key) {
...
AesCbcSubtle.prototype.decrypt = function (data, iv) {
	return subtle
		.decrypt({ name: "AES-CBC", iv: iv }, this.key, data)
		.catch(function () {
...
AesCbcNode.prototype.decrypt = function (data, iv) {
	...
	return Promise.resolve()
		.then(function () {
			...
			var block = cipher.update(Buffer.from(data));
			return ByteUtils.arrayToBuffer(
				Buffer.concat([block, cipher.final()])
			);
		})
		.catch(function () {
			throw new KdbxError(Consts.ErrorCodes.InvalidKey, "invalid key");
